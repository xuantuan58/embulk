...
public class JRubyScriptingModule implements Module {
    ...
    private static class ScriptingContainerProvider
        ...
        @Inject
        public ScriptingContainerProvider(final Injector injector, final SystemConfigProperties systemConfig) {
            // use_global_ruby_runtime is valid only when it's guaranteed that just one Injector is
            // instantiated in this JVM.
            this.useGlobalRubyRuntime = systemConfig.getPropertyAsBoolean("use_global_ruby_runtime", false);

            this.initializer = JRubyInitializer.of(
                    injector,
                    injector.getInstance(ILoggerFactory.class).getLogger("init"),

                    systemConfig.getProperty("gem_home", null),
                    systemConfig.getProperty("gem_path", null),
                    systemConfig.getProperty("jruby_use_default_embulk_gem_home", "false").equals("true"),

                    // TODO get jruby-home from systemConfig to call jruby.container.setHomeDirectory
                    systemConfig.getProperty("jruby_load_path", null),
                    systemConfig.getProperty("jruby_classpath", null),
                    systemConfig.getProperty("jruby_command_line_options", null),

                    systemConfig.getProperty("jruby_global_bundler_plugin_source_directory", null));
        }
