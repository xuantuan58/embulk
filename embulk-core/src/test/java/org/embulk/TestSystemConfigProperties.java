...
public class TestSystemConfigProperties {
    @Test
    ...
    private static void assertInteger(final String text, final int expectedValue, final int defaultValue) {
        ...
        assertEquals(expectedValue, SystemConfigProperties.of(properties).getPropertyAsInteger("key", defaultValue));
    }
    ...
    private static void assertIntegerInvalidFormat(final String text) {
        ...
        properties.setProperty("key", text);
        try {
            final int unexpectedResult = SystemConfigProperties.of(properties).getPropertyAsInteger("key", 123456);
    ...
    private static void assertBoolean(final String text, final boolean expectedValue, final boolean defaultValue) {
        final Properties properties = new Properties();
        properties.setProperty("key", text);
        assertEquals(expectedValue, SystemConfigProperties.of(properties).getPropertyAsBoolean("key", defaultValue));
    }
    ...
    private static void assertBooleanInvalidFormat(final String text) {
        ...
        properties.setProperty("key", text);
        try {
            final boolean unexpectedResult = SystemConfigProperties.of(properties).getPropertyAsBoolean("key", false);
            fail("\"" + text + "\" was unexpectedly parsed successfully: " + unexpectedResult);
        } catch (final IllegalArgumentException ex) {
            return;  // Success.
        }
